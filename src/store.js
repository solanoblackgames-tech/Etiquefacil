...
export async function updateOperatorLargeQrLabelAccess({ ownerUserId, operatorUserId, largeQrLabelAccess }) {
  await ensureStore();
  if (hasPostgres()) {
    const result = await query(
      "update users set large_qr_label_access = $3 where id = $2 and parent_user_id = $1 returning *",
      [ownerUserId, operatorUserId, Boolean(largeQrLabelAccess)]
    );
    if (!result.rows.length) throw notFound("Operador nao encontrado.");
    return { user: sanitizeUser(userFromRow(result.rows[0])) };
  }
...
async function hydrateTriageInputPrice(userId, item) {
  ...
  if (hasPostgres()) {
    ...
    const previousPrice = Number(result.rows[0]?.valor_unit || 0);
    return previousPrice > 0 ? { ...item, valorUnit: roundMoney(previousPrice) } : item;
  }
...
export async function addDiverseLotItem({ userId, createdByUserId = userId, operatorUserId = null, lotId, codigoMl, codigoRz, manualProduct, valorUnitOverride, quantidade = 1, preview = false, allowDuplicate = false }) {
  ...
  if (previousHistory) {
    const reservation = ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
    const { product, item } = buildDiverseLotRecords(lot, previousHistory, normalizedMl, normalizedRz, { valorUnitOverride, createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
    consumeSkuReservationInDb(reservation, product.id);
    db.products.push(product);
    db.rzItems.push(item);
    ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
  ...
  if (!history && source) {
    const reservation = ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
    const { product, item } = buildDiverseLotRecords(lot, source, normalizedMl, normalizedRz, { valorUnitOverride, createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
    consumeSkuReservationInDb(reservation, product.id);
    db.products.push(product);
    db.rzItems.push(item);
    ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
  ...
  if (!history && manualProduct) {
    const sourceManual = normalizeManualProduct(manualProduct, normalizedMl);
    const reservation = ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
    const { product, item } = buildDiverseLotRecords(lot, sourceManual, normalizedMl, normalizedRz, { origem: "lote_sem_planilha_manual", createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
    consumeSkuReservationInDb(reservation, product.id);
    db.products.push(product);
    db.rzItems.push(item);
    mergePendingCatalogRequest(db.catalogRequests, buildCatalogRequest({ userId, createdByUserId, operatorUserId, lot, product, type: "create", payload: sourceManual }));
  ...
  const reservation = ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
  const { product, item } = buildDiverseLotRecords(lot, history, normalizedMl, normalizedRz, { valorUnitOverride, createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
  consumeSkuReservationInDb(reservation, product.id);
  db.products.push(product);
  db.rzItems.push(item);
  ensureActiveSkuReservationInDb(db, { userId, lot, operatorUserId: reservationOperatorId });
...
async function ensurePgStore() {
  ...
  await query(`
    ...
    alter table catalog_requests add column if not exists ncm text not null default '';
    alter table catalog_requests add column if not exists data_validade text not null default '';
    ...
    alter table catalog_rejected_requests add column if not exists ncm text not null default '';
    alter table catalog_rejected_requests add column if not exists data_validade text not null default '';
...
async function insertCatalogProductRows(client, products = []) {
  await insertRows(
    client,
    "catalog_products",
    ["id", "codigo_ml", "descricao", "valor_unit", "preco_custo", "categoria", "subcategoria", "ncm", "ean", "data_validade", "link", "foto", "altura_caixa", "largura_caixa", "comprimento_caixa", "peso_caixa", "localizacao_estoque", "created_at", "updated_at"],
    products.map((product) => [
      product.id,
      product.codigoMl,
      product.descricao,
  ...
  try {
    ...
    if (existing) {
      ...
      if (!history && manualProduct) {
        const source = normalizeManualProduct(manualProduct, codigoMl);
        const reservation = await ensureActiveSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId });
        const records = buildDiverseLotRecords(lot, source, codigoMl, codigoRz, { origem: "lote_sem_planilha_manual", createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
        await insertLotRows(client, { products: [records.product], rzItems: [records.item] });
        await mergePendingCatalogRequestPg(client, buildCatalogRequest({ userId, createdByUserId, operatorUserId, lot, product: records.product, type: "create", payload: source }));
        await consumeSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId, productId: records.product.id });
        await ensureActiveSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId });
      ...
      const reservation = await ensureActiveSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId });
      const records = buildDiverseLotRecords(lot, history, codigoMl, codigoRz, { valorUnitOverride, createdByUserId, operatorUserId, sku: reservation.sku, quantidade: expectedQuantity, qtdConferida: 1 });
      await insertLotRows(client, { products: [records.product], rzItems: [records.item] });
      await consumeSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId, productId: records.product.id });
      await ensureActiveSkuReservationInPg(client, { userId, lot, operatorUserId: reservationOperatorId });
...
async function findPgProductHistory(client, userId, currentLotId, codigoMl, limit) {
  const result = await client.query(
    `
      select
        ...
        cp.ncm as catalog__ncm,
        cp.ean as catalog__ean,
        cp.data_validade as catalog__data_validade,
  ...
  return result.rows.map((row) => {
    ...
    return {
      ...
      ncm: row.catalog__ncm || "",
      ean: row.catalog__ean || "",
      dataValidade: row.catalog__data_validade || "",
...
async function reviewCatalogRequestPg(requestId, action, options = {}) {
  ...
  try {
    ...
    if (action === "approve") {
      ...
      await client.query(
        `
          insert into catalog_products (id, codigo_ml, descricao, valor_unit, preco_custo, categoria, subcategoria, ncm, ean, data_validade, link, foto, altura_caixa, largura_caixa, comprimento_caixa, peso_caixa, localizacao_estoque, created_at, updated_at)
          ...
          on conflict (codigo_ml) do update set
            ...
            updated_at = now()
        `,
        [randomUUID(), selected.codigoMl, selected.descricao, selected.valorUnit, selected.precoCusto || 0, selected.categoria || "", selected.subcategoria || "", selected.ncm || "", selected.ean || "", selected.dataValidade || "", selected.link || "", selected.foto || "", selected.alturaCaixa || null, selected.larguraCaixa || null, selected.comprimentoCaixa || null, selected.pesoCaixa || null, selected.localizacaoEstoque || ""]
      );
...
function buildExternalExcessRecords(lot, history, codigoRz, codigoMl, options = {}) {
  ...
  const product = {
    ...
    ncm: history.ncm || "",
    ean: history.ean || "",
    dataValidade: history.dataValidade || "",
    link: history.link || "",
    foto: history.foto || "",
...
function buildDiverseLotRecords(lot, history, codigoMl, codigoRz, options = {}) {
  ...
  const item = buildDiverseRzItem(lot, product, codigoRz, { quantidade, qtdConferida: options.qtdConferida });
  return { product, item };
}
...
function buildDiverseRzItem(lot, product, codigoRz, options = {}) {
  const quantidade = normalizeExpectedQuantity(options.quantidade);
  const qtdConferida = Math.min(quantidade, Math.max(0, requiredInt(options.qtdConferida ?? 0)));
  return {
    id: randomUUID(),
    lotId: lot.id,
    productId: product.id,
...
function normalizeManualProduct(input = {}, codigoMl) {
  ...
  return {
    ...
    dataValidade: normalizeDateText(input.dataValidade ?? input.data_validade ?? input.validade),
...
function buildCatalogRequest({ userId, createdByUserId = userId, operatorUserId = null, lot, product, type, payload }) {
  ...
  return {
    ...
    ncm: normalizeNcmText(payload.ncm ?? product.ncm),
    ean: payload.ean || product.ean || "",
    dataValidade: normalizeDateText(payload.dataValidade ?? product.dataValidade),
...
function triageStatMoney(...values) {
  for (const value of values) {
    ...
    if (Number.isFinite(number) && number > 0) return roundMoney(number);
  }
  return 0;
}
