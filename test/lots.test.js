...
test("updateLotProduct keeps the entry lot cost unchanged", async () => {
  const originalCwd = process.cwd();
  const originalDatabaseUrl = process.env.DATABASE_URL;
  const tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "etiquefacil-update-product-cost-"));
  ...
  try {
    const storeUrl = pathToFileURL(path.join(originalCwd, "src", "store.js"));
    storeUrl.search = `?test=${Date.now()}-update-product-cost`;
    const { readDb, updateLotProduct, writeDb } = await import(storeUrl.href);

    await writeDb({
      users: [{ id: "user-1", name: "Usuario", email: "u@example.com" }],
      lots: [{ id: "lot-1", userId: "user-1", nomeArquivo: "Lote", createdAt: "2026-07-18T00:00:00.000Z" }],
      products: [
        {
          id: "product-1",
          lotId: "lot-1",
    ...
    const db = await readDb();
    assert.equal(result.product.valorUnit, 15);
    assert.equal(result.product.precoCusto, 2.5);
    assert.equal(db.products[0].precoCusto, 2.5);
  } finally {
    process.chdir(originalCwd);
